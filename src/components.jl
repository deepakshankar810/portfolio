...
function render_edu(edu)
    ...
    <div class="mb-8 ml-6 group">
        ...
        <div class="bg-white dark:bg-slate-800 p-5 rounded-lg shadow-sm border border-gray-200 dark:border-slate-700 hover:shadow-lg hover:border-teal-200 dark:hover:border-teal-500/30 transition transform hover:scale-[1.02]">
...
function render_skill_card(i, skill)
    """
    <div onclick="openModal('skill_$(i)')" class="bg-white/90 dark:bg-slate-800/50 backdrop-blur-md border border-slate-200 dark:border-slate-700 p-8 rounded-2xl cursor-pointer group transition-all duration-300 hover:-translate-y-2 hover:shadow-xl hover:shadow-teal-100 dark:hover:shadow-teal-900/20">
...
function render_project_card(proj)
    """
    <div onclick="openModal('$(proj["id"])')" class="bg-white/90 dark:bg-slate-800/50 backdrop-blur-md border border-slate-200 dark:border-slate-700 p-8 rounded-2xl relative overflow-hidden group cursor-pointer border-t-4 border-t-teal-500 hover:shadow-xl hover:shadow-teal-100 dark:hover:shadow-teal-900/20 transition-all duration-300">
...
function render_manuscript_card(manu)
    border_color = manu["type"] == "Published" ? "border-l-green-500" : "border-l-purple-500"
    badge_class = manu["type"] == "Published" ? 
    ...
    """
    <div onclick="openModal('$(manu["id"])')" class="bg-white/90 dark:bg-slate-800/50 backdrop-blur-md border border-slate-200 dark:border-slate-700 p-6 rounded-xl cursor-pointer hover:bg-white dark:hover:bg-slate-800 hover:shadow-lg transition border-l-4 $border_color flex flex-col h-full hover:-translate-y-1">
...
function render_cert_card(cert)
    """
    <div onclick="openModal('$(cert["id"])')" class="bg-white/90 dark:bg-slate-800/50 backdrop-blur-md border border-slate-200 dark:border-slate-700 p-6 rounded-xl text-center cursor-pointer shadow-sm hover:shadow-lg hover:shadow-teal-100 dark:hover:shadow-teal-900/20 hover:-translate-y-1 transition h-full flex flex-col items-center justify-center">
